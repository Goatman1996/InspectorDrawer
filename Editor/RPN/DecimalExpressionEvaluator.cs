...
namespace GMToolKit
{
    public class DecimalExpressionEvaluator
    {
        ...
        public static bool EvaluateDecimal(string expression, out decimal result)
        {
            ...
            return EvaluateDecimal(rpnTokens, out result);
        }

        private static bool EvaluateDecimal(string[] tokens, out decimal value)
        {
            ...
            foreach (string token in tokens)
            {
                if (IsOperator(token))
                {
                    ...
                    {
                        decimal newValue;
                        parsed &= TryParseDecimal(stack.Pop(), out newValue);
                        values.Add(newValue);
                    }
                    values.Reverse();

                    if (parsed && values.Count == oper.inputs)
                        stack.Push(EvaluateDecimal(values.ToArray(), token[0]).ToString());
            ...
            if (stack.Count == 1)
            {
                if (TryParseDecimal(stack.Pop(), out value))
                    return true;
            }
        ...
        private static bool TryParseDecimal(string expression, out decimal result)
        {
            expression = expression.Replace(',', '.');
            expression = expression.TrimEnd('m');
            expression = expression.TrimEnd('M');

            bool success = false;
            result = default;
            decimal temp = default;
            success = decimal.TryParse(expression, NumberStyles.Float, CultureInfo.InvariantCulture.NumberFormat, out temp);
            result = temp;
            return success;
        }

        private static decimal EvaluateDecimal(decimal[] values, char oper)
        {
            if (values.Length == 1)
            {
                switch (oper)
